...
const ShowAllServiceHistory = () => {
    ...
    const fetchServiceHistories = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8077/ServiceHistory"
        );
        setServiceHistories(response.data.service);
        setFilteredHistories(response.data.service);
      } catch (error) {
        console.error(
          "There was an error fetching the service histories!",
          error
        );
      } finally {
        setLoading(false);
  ...
  const handleSearch = (event) => {
    ...
    const filtered = serviceHistories.filter(
      (history) =>
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Are you sure you want to delete this service history?")
    ) {
      try {
        await axios.delete(`http://localhost:8077/ServiceHistory/${id}`);
        setServiceHistories(
          serviceHistories.filter((history) => history._id !== id)
        );
        setFilteredHistories(
          filteredHistories.filter((history) => history._id !== id)
        );
        alert("Service history deleted successfully!");
      } catch (error) {
        console.error(
          "There was an error deleting the service history!",
          error
        );
